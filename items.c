...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    //这边就可以得到slabs_class上第一个添加的item的地址
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        if (hv == cur_hv || (hold_lock = item_trylock(hv)) == NULL) //给search的item上锁，无法修改该数据
            continue;
        ...
        /* Expired or flushed */
        //这边判断尾部的Item是否过期，如果已经过期，将当前的过期的item分配给最新的缓存
        if ((search->exptime != 0 && search->exptime < current_time)
            ...
            it = search; //直接记录下已过期的item
            slabs_adjust_mem_requested(it->slabs_clsid, ITEM_ntotal(it), ntotal);
            do_item_unlink_nolock(it, hv); //将it从LRU链表中删除
            /* Initialize the item block: */
            it->slabs_clsid = 0;
        } else if ((it = slabs_alloc(ntotal, id)) == NULL) {  //slabs_alloc方法是去分配一个新的内存块
        	 //此刻，过期失效的item没有找到，申请内存又失败了
        	//只能用LRU淘汰一个item(即使这个item并没有过期失效)

        		tried_alloc = 1;
            if (settings.evict_to_free == 0) { //若没设置LRU淘汰则返回错误
                ...
                if ((search->it_flags & ITEM_FETCHED) == 0) {
                	...
                	//这边直接将LRU尾部的ITEM淘汰，保存尾部ITEM地址并且给最新的ITEM使用
                it = search;
    ...
    it->nkey = nkey;
    it->nbytes = nbytes;
    memcpy(ITEM_key(it), key, nkey); //这边是内存拷贝，拷贝到item结构地址的内存块上
    it->exptime = exptime;
    memcpy(ITEM_suffix(it), suffix, (size_t)nsuffix); //这边也是内存拷贝
    it->nsuffix = nsuffix;
    return it;
}
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    assoc_insert(it, hv); //分配到HashTable的桶上
    //LRU链
    item_link_q(it);
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    //mutex_lock(&cache_lock);
    item *it = assoc_find(key, nkey, hv); //查询HashTable上的Item
    if (it != NULL) {
    ...
    if (it != NULL) {
    	...
    	//有两种删除的原因：1.超过flush执行命令的时间；2.超过预设的过期时间
