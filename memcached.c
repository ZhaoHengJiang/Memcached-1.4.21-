...
//初始化一个新连接，注册该连接事件，设置事件类型是event_flags，回调函数是event_handler：据连接状态处理连接
conn *conn_new(const int sfd, enum conn_states init_state,
	...
	if (NULL == c) {
		...
		c->iovsize = IOV_LIST_INITIAL; //系统会分配400个iovec的结构，最高水位600个
	...
	//注册该连接事件，设置事件类型是event_flags，回调函数是event_handler：据连接状态处理连接
	//主线程主要是监听并处理用户的socket连接事件；worker线程主要监听并处理socket的读写事件
 ...
 * buffers!
 *
 * 如果连接的缓冲区太大，则缩小它们。 这样可以防止一次大量数据的“get”请求永久性地占用大量服务器内存。非常优秀的设计
 *
 * 只能在请求之间调用此方法，因为该方法会清除输出缓冲区！
 */
 ...
 * Returns 0 on success, -1 on out-of-memory.
 */
//将Memcached需要发送的数据，分成N多个IOV的块
//将IOV块添加到msghdr的结构中去。
static int add_iov(conn *c, const void *buf, int len) {
	struct msghdr *m;
	int leftover; //UDP传送结束的标识
	bool limit_to_mtu;
	...
	do {
		...
		if (m->msg_iovlen == IOV_MAX || (limit_to_mtu && c->msgbytes
...
static int build_udp_headers(conn *c) {
	...
	for (i = 0; i < c->msgused; i++) {
		...
		*hdr++ = 0;
		assert(
				(void *) hdr == (caddr_t) c->msglist[i].msg_iov[0].iov_base
						+ UDP_HEADER_SIZE);
	}
...
static void write_bin_response(conn *c, void *d, int hlen, int keylen, int dlen) {
	if (!c->noreply || c->cmd == PROTOCOL_BINARY_CMD_GET || c->cmd
	...
	case DELTA_ITEM_NOT_FOUND:
		if (req->message.body.expiration != 0xffffffff) {
			...
			it = item_alloc(key, nkey, 0,
					realtime(req->message.body.expiration), res + 2);
...
static void process_bin_get_or_touch(conn *c) {
	...
	int should_touch = (c->cmd == PROTOCOL_BINARY_CMD_TOUCH || c->cmd
			== PROTOCOL_BINARY_CMD_GAT || c->cmd == PROTOCOL_BINARY_CMD_GATK);
	int should_return_key = (c->cmd == PROTOCOL_BINARY_CMD_GETK || c->cmd
...
static void append_bin_stats(const char *key, const uint16_t klen,
	...
	protocol_binary_response_header header = { .response.magic =
			...
			.response.keylen = (uint16_t) htons(klen), .response.datatype =
					(uint8_t) PROTOCOL_BINARY_RAW_BYTES, .response.bodylen =
					htonl(bodylen), .response.opaque = c->opaque };
...
static void complete_nread_binary(conn *c) {
	...
	case bin_reading_set_header:
		if (c->cmd == PROTOCOL_BINARY_CMD_APPEND || c->cmd
...
static size_t tokenize_command(char *command, token_t *tokens,
	...
	size_t len = strlen(command); //命令长度，因为是以'\0'为分隔，因此命令长度为一条命令的长度，设计巧妙
	unsigned int i = 0;
...
static inline bool set_noreply_maybe(conn *c, token_t *tokens, size_t ntokens) {
	...
	/*
	 ...
	 it can't be helped.
	 */
	if (tokens[noreply_index].value && strcmp(tokens[noreply_index].value,
			"noreply") == 0) {
		c->noreply = true;
	}
	return c->noreply;
...
static void conn_to_str(const conn *c, char *buf) {
	...
	if (!c) {
		...
		if (c->state == conn_listening || (IS_UDP(c->transport) && c->state
				== conn_read)) {
		...
		switch (af) {
		case AF_INET:
			(void) inet_ntop(af, &((struct sockaddr_in *) addr)->sin_addr,
					addr_text, sizeof(addr_text) - 1);
		...
		case AF_INET6:
			addr_text[0] = '[';
			addr_text[1] = '\0';
			if (inet_ntop(af, &((struct sockaddr_in6 *) addr)->sin6_addr,
		...
		case AF_UNIX:
			strncpy(addr_text, ((struct sockaddr_un *) addr)->sun_path,
					sizeof(addr_text) - 1);
...
static void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
	...
	if (ntokens == 2) {
		...
		if (!safe_strtoul(tokens[2].value, &id) || !safe_strtoul(
				tokens[3].value, &limit)) {
...
static inline void process_get_command(conn *c, token_t *tokens,
	...
	token_t *key_token = &tokens[KEY_TOKEN]; //KEY_TOKEN=1 因此循环开始是从第一个key值开始
	char *suffix;
	assert(c != NULL);
	...
	do {
		...
		while (key_token->length != 0) {
			...
			//判断key的长度是否超过了最大的长度，memcached key的最大长度为250
			...
			if (it) {
				...
				if (i >= c->isize) {
					...
					if (new_list) {
						c->isize *= 2; //存放需要向客户端写数据的item的列表的长度
						c->ilist = new_list;
					} else {
						STATS_LOCK();
						stats.malloc_fails++;
				...
				if (return_cas) {
					...
					*(c->suffixlist + i) = suffix;
					...
					if (add_iov(c, "VALUE ", 6) != 0
							...
							|| add_iov(c, ITEM_data(it), it->nbytes) != 0) { //add_iov()将数据添加至待输出链表
						item_remove(it);
						break;
					}
...
static void process_update_command(conn *c, token_t *tokens,
	...
	//检查参数的合法性
	if (!(safe_strtoul(tokens[2].value, (uint32_t *) &flags) && safe_strtol(
			tokens[3].value, &exptime_int) && safe_strtol(tokens[4].value,
			(int32_t *) &vlen))) {
		out_string(c, "CLIENT_ERROR bad command line format");
		return;
	}
...
//命令处理函数
//try_read_command()中，我们找到了rbuf中\n的字符，然后将其替换成'\0'
static void process_command(conn *c, char *command) {

	//tokens结构，这边会将c->rcurr命令拆分出来
	//并且将命令通过空格符号来分隔成多个元素
	//例如：set username zhuli,则会拆分成3个元素，分别是set和username和zhuli
	//token_t结构体是二维数组，定义如下
	//		typedef struct token_s {
	//	  	 	 char *value;
	//    	 size_t length;
	//		} token_t;
	...
	//tokenize_command()拆分一个命令
	//并且将拆分出来的命令元素放进tokens的数组中
	//参数:command为命令
	ntokens = tokenize_command(command, tokens, MAX_TOKENS);

	//COMMAND_TOKEN=0，因此tokens[COMMAND_TOKEN]为命令
	//分解出来的命令的第一个参数为操作方法
	//根据命令有多少个单词来分别判断是什么命令
	if (ntokens >= 3 && ((strcmp(tokens[COMMAND_TOKEN].value, "get") == 0)
		...
		//处理add/set/replace等添加命令
		process_update_command(c, tokens, ntokens, comm, false);

	} else if ((ntokens == 7 || ntokens == 8) && (strcmp(
			tokens[COMMAND_TOKEN].value, "cas") == 0 && (comm = NREAD_CAS))) {
	...
	} else if (ntokens >= 3 && (strcmp(tokens[COMMAND_TOKEN].value, "gets")
			== 0)) {

		process_get_command(c, tokens, ntokens, true);

	} else if ((ntokens == 4 || ntokens == 5) && (strcmp(
			tokens[COMMAND_TOKEN].value, "decr") == 0)) {
		...
		process_delete_command(c, tokens, ntokens);
			...
			tokens[COMMAND_TOKEN].value, "touch") == 0)) {
	...
	} else if (ntokens >= 2 && ntokens <= 4 && (strcmp(
			tokens[COMMAND_TOKEN].value, "flush_all") == 0)) {
		time_t exptime = 0;
 ...
 *
 * 这个方法主要是用来读取c->rbuf中的命令的。例如命令：set username zhuli\r\n get username \n
 * 则会通过\n这个换行符来分隔数据报文中的命令。因为数据报文会有粘包和拆包的特性，所以只有等到命令行完整了才能进行解析。所有只有匹配到了\n符号，才能匹配一个完整的命令。
 * 如果c->buf内存块中的数据匹配不到\n，则返回0继续等待客户端的命令数据报文到来conn_waiting；否则就会转交给process_command方法，来处理具体的命令（命令解析会通过\0符号来分隔）
 */
static int try_read_command(conn *c) {
	assert(c != NULL);
	...
	if (c->protocol == binary_prot) {
		...
		if (!el) {
			...
			if (c->rbytes > 1024) {
				...
				if (ptr - c->rcurr > 100 || (strncmp(ptr, "get ", 4)
						&& strncmp(ptr, "gets ", 5))) {
		...
		*el = '\0';
		...
		process_command(c, c->rcurr); //处理具体的命令
 ...
 *
 * 读取TCP网络客户端传递过来的命令数据。读取到的数据会放进c->rbuf中
...
static enum try_read_result try_read_network(conn *c) {
	...
	while (1) {
		...
		if (c->rbytes >= c->rsize) {
			//若已重新分配四次内存，则不再重新分配，需要新开辟一块rbuf
			if (num_allocs == 4) {
				return gotdata;
			}
			++num_allocs;
			...
			//若分配失败则提示客户端错误并返回READ_MEMORY_ERROR
			if (!new_rbuf) {
				STATS_LOCK();
				stats.malloc_fails++;
				STATS_UNLOCK();
...
//更新连接事件的类型
//1. 判断是否需要修改，若不需要修改则直接返回
//2. 若需要修改删除原连接事件
//3. 将新的连接事件重新注册到event_base上
//4. 将新的连接事件添加至监听映射evmap
static bool update_event(conn *c, const int new_flags) {
	assert(c != NULL);
	...
	if (event_del(&c->event) == -1)
		return false;
	event_set(&c->event, c->sfd, new_flags, event_handler, (void *) c); //初始化新事件
	event_base_set(base, &c->event); //将事件注册到event_base上
	c->ev_flags = new_flags;
	if (event_add(&c->event, 0) == -1)
		return false; //将事件添加至监听映射
	return true;
}
 ...
 */
//这个方法主要向客户端写数据，返回transmit_result的枚举类型
...
//监听socket连接、监听socket的读写、断开连接等操作都是在drive_machine这个方法中实现的。
//而这些操作都是通过c->state这个状态来判断不同的操作类型。
static void drive_machine(conn *c) {
	bool stop = false;
	int sfd;
	socklen_t addrlen;
	...
	while (!stop) {
		...
		/***********************************监听socket连接*************************************/
		//1. accpet()监听描述c->sfd返回得到连接描述符sfd，建立连接
		//2. 判断连接数量是否超设定值
		//3. 若没超过设定值，则将新连接的信息分发到worker线程中进行处理

		//conn_listening状态只有main线程才能获得
		...
		case conn_listening:
			...
			//判断连接数是否过多
			if (settings.maxconns_fast && stats.curr_conns + stats.reserved_fds
					>= settings.maxconns - 1) {
				...
				//这个函数会将连接信息分发到某一个worker线程中，然后worker线程接管具体的读写操作
		...
		case conn_waiting:
			...
			//1. 判断是否是UDP连接，是的话调用try_read_udp()读取数据，不是调用try_read_network()读取数据
			//2. 根据返回的res修改连接状态
		case conn_read:
			res = IS_UDP(c->transport) ? try_read_udp(c) : try_read_network(c); //读取到的数据会存储在c->rbuf容器中
			...
			/****************************解析命令行********************************/
			//调用try_read_command()解析命令
		case conn_parse_cmd:
			if (try_read_command(c) == 0) { //读取c->rbuf中的命令数据，通过\n来分隔数据报文的命令，如果c->buf内存块中的数据匹配不到\n返回0，继续等待客户端的命令数据报文到来
				/* wee need more data! */
				conn_set_state(c, conn_waiting);
			}
		...
		case conn_new_cmd:
			...
			if (nreqs >= 0) {
				/*
				 * reset_cmd_handler(c)判断c->rbytes中是否还有剩余的报文没处理，如果未处理，则转交到conn_parse_cmd（第四步）继续解析剩余命令；
				 * 如果已经处理了，则转交到conn_waiting，等待新的事件到来。在转交之前，每次都会执行一次conn_shrink方法
				 */
				reset_cmd_handler(c);
			} else {
				pthread_mutex_lock(&c->thread->stats.mutex);
			...
			/****************************读取缓存的value值，将数据拷贝到item数据结构********************************/
		case conn_nread:
			//缓存 value数据报文的长度为0的时候，说明已经读取完成了，进行下一步操作
			if (c->rlbytes == 0) {
				//调用存储数据store_item的方法
			...
			if (settings.verbose > 0) {
				fprintf(stderr, "Failed to read, and not due to blocking:\n"
					...
					"rcurr=%lx ritem=%lx rbuf=%lx rlbytes=%d rsize=%d\n",
						errno, strerror(errno), (long) c->rcurr,
		...
		case conn_write:
			...
			/****************************向客户端写数据********************************/
			//写完数据后，如果写失败，则关闭连接；如果写成功，则会将状态修改成conn_new_cmd，继续解析c->rbuf中剩余的命令
		case conn_mwrite:
			if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c)
					!= 0) {
				if (settings.verbose > 0)
			...
			case TRANSMIT_COMPLETE:
				if (c->state == conn_mwrite) {
					...
					if (c->protocol == binary_prot) {
						...
						conn_set_state(c, conn_new_cmd); //如果写入成功，则转交到conn_new_cmd这个状态
					}
 ...
 */
//创建一个socket并bind端口号
static int server_socket(const char *interface, //需要绑定的接口
		int port, //需要绑定的端口号
		enum network_transport transport, //传输协议TCP/UDP
	...
	for (next = ai; next; next = next->ai_next) {
		...
		if (IS_UDP(transport)) {
			...
			//创建一个新的事件
			//区别于worker线程，main线程的方法指定了state的类型为：conn_listening
...
static int server_sockets(int port, enum network_transport transport,
	...
	if (settings.inter == NULL) {
		...
		for (char *p = strtok_r(list, ";,", &b); p != NULL; p = strtok_r(NULL,
				";,", &b)) {
			int the_port = port;
			char *s = strchr(p, ':');
			if (s != NULL) {
...
int main(int argc, char **argv) {
	...
	char * const subopts_tokens[] = { [MAXCONNS_FAST] = "maxconns_fast",
			...
			[SLAB_AUTOMOVE] = "slab_automove", [TAIL_REPAIR_TIME
					] = "tail_repair_time",
			[HASH_ALGORITHM] = "hash_algorithm", [LRU_CRAWLER] = "lru_crawler",
			[LRU_CRAWLER_SLEEP] = "lru_crawler_sleep", [LRU_CRAWLER_TOCRAWL
					] = "lru_crawler_tocrawl", NULL };
		...
		case 'o': /* It's sub-opts time! */
			...
			while (*subopts != '\0') {
				...
				case SLAB_AUTOMOVE:
					...
					if (settings.slab_automove < 0 || settings.slab_automove
							> 2) {
						fprintf(stderr,
				...
				case TAIL_REPAIR_TIME:
					if (subopts_value == NULL) {
						fprintf(stderr,
								"Missing numeric argument for tail_repair_time\n");
						return 1;
					}
				...
				case HASH_ALGORITHM:
					...
					if (strcmp(subopts_value, "jenkins") == 0) {
						...
						fprintf(stderr,
								"Unknown hash_algorithm option (jenkins, murmur3)\n");
						return 1;
					}
					break;
				case LRU_CRAWLER:
	...
	if (settings.socketpath == NULL) {
		...
		if (portnumber_filename != NULL) {
			snprintf(temp_portnumber_filename,
					sizeof(temp_portnumber_filename), "%s.lck",
					portnumber_filename);
