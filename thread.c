 ...
 * 1. 初始化worker线程的event_base，注册添加异步事件，事件类型是EV_READ|EV_PERSIST，回调函数是thread_libevent_process：处理新的连接
 * 2. 初始化worker线程的new_conn_queue
 */
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    //注册异步事件，设置事件类型是EV_READ|EV_PERSIST，回调函数是thread_libevent_process：处理新的连接
 ...
 * main_thread创建了worker_thread时的回调函数
 * 开启libevent的事件循环
 */
 ...
 * 当管道接收到数据时，回调该函数处理新的连接
 * 1. 判断连接是否正确
 * 2. 取出该连接的CQ_ITEM连接信息
 * 3. 初始化该连接的其他信息，并注册添加“根据状态处理新连接”事件
 */
static void thread_libevent_process(int fd, short which, void *arg) {
    LIBEVENT_THREAD *me = arg;
    CQ_ITEM *item;
    ...
    //回调函数中回去读取pipe中的信息
    //主线程中如果有新的连接，会向其中一个线程的pipe中写入'c'
    //这边读取pipe中的数据，如果返回1，则说明从pipe中获取的数据是正确的
    if (read(fd, buf, 1) != 1)
        if (settings.verbose > 0)
            fprintf(stderr, "Can't read from libevent pipe\n");

    switch (buf[0]) {
    //若buf中存放的是'c'，代表是一个新连接
    case 'c':
    ...
    if (NULL != item) {
    	//conn_new()初始化新连接的其他信息，注册添加新连接事件，设置事件类型是event_flags，回调函数是event_handler：据连接状态处理连接
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    //这边通过循环的方式创建nthreads个线程的管道和event_base
   //nthreads应该是可以设置的
    for (i = 0; i < nthreads; i++) {
        ...
        //创建每个线程自己的libevent的event_base，注册添加新连接事件，初始化new_conn_queue队列
